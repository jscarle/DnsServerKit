...
public sealed class DnsName : IEquatable<DnsName>
{
    ...
    public DnsName(string value)
    {
        ...
        var wireLength = Encode(value, wireBytes);

        Value = value.Length == 0 ? "." : value;
        _wireBytes = wireBytes[..wireLength].ToArray();
    ...
    internal static int StartWireHash() => unchecked((int)HashOffset);
    ...
    internal static int ComputeQuestionHash(int nameHashCode, ushort type, ushort @class)
    {
        var hashCode = AppendWireHash(nameHashCode, (byte)(type >> 8));
        hashCode = AppendWireHash(hashCode, (byte)type);
        hashCode = AppendWireHash(hashCode, (byte)(@class >> 8));
    ...
    public bool Equals(DnsName? other)
    {
        ...
        return other is not null
               ...
               && _wireBytes.AsSpan().SequenceEqual(other._wireBytes);
    }

    public override bool Equals(object? obj) => obj is DnsName other && Equals(other);

    public override int GetHashCode() => WireHashCode;

    public override string ToString() => Value;

    private static int Encode(string value, Span<byte> destination)
    {
        if (value.Length == 0 || value.Equals(".", StringComparison.Ordinal))
        {
            destination[0] = 0;
            return 1;
        }
        ...
        while (characterIndex < value.Length)
        {
            ...
            if (character == '\\')
            {
                ...
                {
                    if (characterIndex + 2 >= value.Length
                        || value[characterIndex + 1] is < '0' or > '9'
                        || value[characterIndex + 2] is < '0' or > '9')
                    {
                    ...
                    var escapedOctet = ((escapedCharacter - '0') * 100)
                                       + ((value[characterIndex + 1] - '0') * 10)
                                       + (value[characterIndex + 2] - '0');
