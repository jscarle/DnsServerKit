...
/// <remarks>
/// The context remains valid until it is reused by <see cref="DnsReader.Read"/>. Its scalar fields and question
/// bytes remain valid while a response is written over the same worker buffer, but <see cref="Datagram"/> must not
/// be treated as the original request after writing begins.
/// </remarks>
public sealed class DnsQueryContext
{
        ...
        int questionEndOffset)
    {
        ...
        Question.Set(nameOffset, nameLength, type, @class, lookupHashCode);
    }
