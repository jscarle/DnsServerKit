...
internal static class DnsReader
{
    ...
    public static DnsReadResult Read(ReadOnlyMemory<byte> datagram, DnsQueryContext queryContext)
    {
        ...
        {
            var notImplementedResponse = new DnsErrorResponse(transactionId, operation, recursionDesired, ResponseCode.NotImplemented);

            return new DnsReadResult(DnsReadOutcome.ErrorResponse, DnsReadFailure.UnsupportedOperation, notImplementedResponse);
        }

        var formatErrorResponse = new DnsErrorResponse(transactionId, operation, recursionDesired, ResponseCode.FormatError);

        try
        {
            if ((flags & 0x04C0) != 0 || (flags & 0x000F) != 0)
                return new DnsReadResult(DnsReadOutcome.ErrorResponse, DnsReadFailure.InvalidHeader, formatErrorResponse);
            ...
            while (true)
            {
                if ((uint)offset >= (uint)span.Length)
                    return new DnsReadResult(DnsReadOutcome.ErrorResponse, DnsReadFailure.InvalidName, formatErrorResponse);
                ...
                if ((labelLength & 0xC0) != 0 || labelLength > span.Length - offset || offset + labelLength - nameOffset + 1 > 255)
                    return new DnsReadResult(DnsReadOutcome.ErrorResponse, DnsReadFailure.InvalidName, formatErrorResponse);
            ...
            if (span.Length - offset < 4)
                return new DnsReadResult(DnsReadOutcome.ErrorResponse, DnsReadFailure.TruncatedQuestion, formatErrorResponse);
            ...
            if (offset != span.Length)
                return new DnsReadResult(DnsReadOutcome.ErrorResponse, DnsReadFailure.TrailingData, formatErrorResponse);

            queryContext.Set(datagram, transactionId, flags, operation, nameOffset, nameLength, type, @class, nameHashCode, offset);

            return new DnsReadResult(DnsReadOutcome.Query);
        }
        catch (Exception exception)
        {
            var serverFailureResponse = new DnsErrorResponse(transactionId, operation, recursionDesired, ResponseCode.ServerFailure);

            return new DnsReadResult(DnsReadOutcome.ErrorResponse, DnsReadFailure.UnexpectedException, serverFailureResponse, exception);
        }
    }
}
